...
STUDENT_TEST("Flag Set Check #1: Attacking the bandit in the tavern.")
{
    ...
    CHECK_FALSE(find_object("bandit").get_object_flag("is_alive").empty()); // alternate style of check

    // Simulate gameplay loop
    parserOutput = {"go to", find_object("abandoned town")};
    main_action(parserOutput.first, (parserOutput.second).get(), player);
    ...
    CHECK_FALSE(find_object("bandit").get_object_flag("is_alive") == "is_alive");
